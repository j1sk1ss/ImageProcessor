...
int BMP_get_pixel_24(int x, int y, bitmap_t* bmp, palette_t* plt) {
    if (!bmp || !plt) return -10;
    ...
    plt->b = bmp->image_data[offset] & 0xFF;
    plt->g = bmp->image_data[offset + 1] & 0xFF;
    plt->r = bmp->image_data[offset + 2] & 0xFF;

    return 1;
}

int BMP_set_pixel_24(int x, int y, bitmap_t* bmp, palette_t* plt) {
    if (!bmp || !plt) return -10;
    if (bmp->iheader.biBitCount != 24) return -1;
    ...
    if (offset + 2 >= bmp->iheader.biSizeImage) return -2;
    bmp->image_data[offset] = plt->b & 0xFF;
    bmp->image_data[offset + 1] = plt->g & 0xFF;
    bmp->image_data[offset + 2] = plt->r & 0xFF;
...
int BMP_resize(bitmap_t* bmp, int nw, int nh, palette_t* plt) {
    ...
    unsigned char* new_image_data = (unsigned char*)malloc(nh * new_row_stride);
